...
0;a<this.onSettledCallbacks_.length;++a)b.asyncExecute(this.onSettledCallbacks_[a]);this.onSettledCallbacks_=null}};var b=new e;g.prototype.settleSameAsPromise_=function(b){var c=this.createResolveAndReject_();b.callWhenSettled_(c.resolve,c.reject)};g.prototype.settleSameAsThenable_=function(b,c){var a=this.createResolveAndReject_();try{b.call(c,a.resolve,a.reject)}catch(l){a.reject(l)}};g.prototype.then=function(b,c){function a(b,c){return"function"==typeof b?function(c){try{e(b(c))}catch(q){d(q)}}:
c}var e,d,f=new g(function(b,c){e=b;d=c});this.callWhenSettled_(a(b,e),a(c,d));return f};g.prototype.catch=function(b){return this.then(void 0,b)};g.prototype.callWhenSettled_=function(a,c){function e(){switch(l.state_){case 1:a(l.result_);break;case 2:c(l.result_);break;default:throw Error("Unexpected state: "+l.state_);}}var l=this;null==this.onSettledCallbacks_?b.asyncExecute(e):this.onSettledCallbacks_.push(e)};g.resolve=d;g.reject=function(b){return new g(function(c,a){a(b)})};g.race=function(b){return new g(function(c,
a){for(var e=$jscomp.makeIterator(b),k=e.next();!k.done;k=e.next())d(k.value).callWhenSettled_(c,a)})};g.all=function(b){var c=$jscomp.makeIterator(b),a=c.next();return a.done?d([]):new g(function(b,e){function l(a){return function(c){k[a]=c;f--;0==f&&b(k)}}var k=[],f=0;do k.push(void 0),f++,d(a.value).callWhenSettled_(l(k.length-1),e),a=c.next();while(!a.done)})};return g},"es6","es3");
...
var k=function(a){return(_(a.match(/^(user|http)/)).isNull()?d+"/":"")+a};c.apply_filters=function(){var a=_.chain(c.structures).where({selected:!0}).pluck("id").value(),b=_.chain(c.groups).where({selected:!0}).pluck("id").value(),d=_.chain(c.grades).where({selected:!0}).pluck("id").value(),e=function(a){return _.chain(c.groups).select(function(b){return _(a).contains(b.id)}).pluck("grades").flatten().pluck("grade_id").uniq().value()},f=function(a){return _.chain(c.groups).select(function(b){return _(a).contains(b.id)}).pluck("structure_id").uniq().value()};
return function(m){return(""+m.firstname+m.lastname).toLocaleLowerCase().includes(c.filters.text.toLocaleLowerCase())&&(0==a.length||0<_(a).intersection(f(_(m.groups).pluck("group_id"))).length)&&(0==b.length||0<_(b).intersection(_(m.groups).pluck("group_id")).length)&&(0==d.length||0<_(d).intersection(e(_(m.groups).pluck("group_id"))).length)&&(!c.only_display_relevant_to||m.relevant)}};c.clear_filters=function(a){_(["CLS","GRP","GPL"]).contains(a)?_.chain(c.groups).select(function(b){return b.selected&&
...
a.data;return c.current_user.get_actual_groups()},function(a){}).then(function(a){var b=[],d=[],m=function(a){c.groups=c.groups.concat(_(a).select(function(a){return"GPL"==a.type||void 0!=_(a.users).findWhere({type:"ELV"})}));c.groups=_(c.groups).uniq(function(a){return a.id});f.get_grades(_.chain(c.groups).pluck("grades").flatten().pluck("grade_id").value()).then(function(a){c.grades=a.data},function(a){});b=b.concat(_.chain(a).pluck("users").flatten().compact().select(function(a){return"ELV"==a.type}).pluck("user_id").value())},
...
m(a.data)}));a=_.chain(c.current_user.profiles).select(function(a){return _("ADM DIR DOC ETA EVS ORI".split(" ")).contains(a.type)}).pluck("structure_id").uniq().value();_(a).isEmpty()||(g(a),d.push(f.get_groups_of_structures(a).then(function(a){m(a.data)})));e.all(d).then(function(){for(b=_(b).uniq();0<b.length;)f.get_users(b.splice(0,200)).then(function(a){c.eleves=c.eleves.concat(_(a.data).map(function(a){a.avatar=k(a.avatar);a.excluded=!1;a.relevant=_(c.relevant_to).contains(a.id);var b=_.chain(a.groups).map(function(a){return _(c.groups).findWhere({id:a.group_id,
...
n=_(g).where({type:"ELV"}),0<n.length?f.get_users(_(n).pluck("user_id")):d.resolve({no_pupils:!0})):d.resolve({no_profile:!0})},function(a){return d.reject(a)}).then(function(b){n=_(n).indexBy("user_id");a.push(_(b.data).map(function(a){var c=1<b.data.length?"s":"";a.type="Autre"+c+" \u00e9l\u00e8ve"+c;return a}));m=_(g).where({type:"ENS"});return 0<m.length?f.get_users(_(m).pluck("user_id")):d.resolve()},function(a){return d.reject(a)}).then(function(b){m=_(m).indexBy("user_id");_(g).where({type:"PRI"});
a.push(_(b.data).map(function(a){a.type=e[m[a.id].type].name+" de l'\u00e9l\u00e8ve";f.get_subjects(_(a.groups).pluck("subject_id")).then(function(b){a.actual_subjects=b.data});return a}));return d.resolve()},function(a){return d.reject(a)}).then(function(){return d.resolve(_.chain(a).flatten().uniq(function(a){return a.id}).value())})});return b}}})}]);
...
resolve:{uids:function(){return b},onglet:function(){return _(g).isNull()?{name:""}:g},all_onglets:function(){return p}},controller:["$scope","$uibModalInstance","$q","Droits","APIs","URL_ENT","DEFAULT_RIGHTS_ONGLET","UID","uids","onglet","all_onglets",function(a,b,d,e,f,g,h,k,l,n,p){a.$ctrl=a;a.uids=l;a.onglet=n;a.all_onglets=p;a.onglet.delete=!1;a.valid_name=!0;_(a.onglet).has("id")||_(a.onglet).has("ids")?e.query({onglet_id:a.onglet.id,"onglets_ids[]":a.onglet.ids}).$promise.then(function(b){a.droits=

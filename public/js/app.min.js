...
0)};e.prototype.executeBatch_=function(){for(;this.batch_&&this.batch_.length;){var a=this.batch_;this.batch_=[];for(var c=0;c<a.length;++c){var b=a[c];a[c]=null;try{b()}catch(m){this.asyncThrow_(m)}}}this.batch_=null};e.prototype.asyncThrow_=function(a){this.asyncExecuteFunction(function(){throw a;})};var g=function(a){this.state_=0;this.result_=void 0;this.onSettledCallbacks_=[];var c=this.createResolveAndReject_();try{a(c.resolve,c.reject)}catch(k){c.reject(k)}};g.prototype.createResolveAndReject_=
...
angular.module("suiviApp").component("onglets",{bindings:{uids:"\x3c"},controller:["$uibModal","Onglets","Popups","APIs","User","UID",function(a,e,d,f,g,b){var h=this;h.popup_onglet=d.onglet;h.callback_popup_onglet=function(a){"created"==a.action&&h.onglets.push(a);a.delete&&(h.onglets=h.onglets.filter(function(c){return c.name!=a.name}))};h.$onInit=function(){1==h.uids.length?d.loading_window("Chargement des onglets","",function(){return e.query({"uids[]":h.uids}).$promise.then(function(a){h.onglets=
...
a.data;return c.current_user.get_actual_groups()},function(a){}).then(function(a){var b=[],d=[],l=function(a){c.groups=c.groups.concat(_(a).select(function(a){return"GPL"==a.type||void 0!=_(a.users).findWhere({type:"ELV"})}));c.groups=_(c.groups).uniq(function(a){return a.id});f.get_grades(_.chain(c.groups).pluck("grades").flatten().pluck("grade_id").value()).then(function(a){c.grades=a.data},function(a){});b=b.concat(_.chain(a).pluck("users").flatten().compact().select(function(a){return"ELV"==a.type}).pluck("user_id").value())},
g=function(a){1<a.length&&f.get_structures(a).then(function(a){c.structures=c.structures.concat(a.data);c.structures=_(c.structures).uniq(function(a){return a.id})})};_.chain(c.current_user.profiles).pluck("type").contains("ELV").value()&&b.push(c.current_user.id);b=b.concat(_(c.current_user.children).pluck("child_id"));a=_.chain(a).reject(function(a){return _(["ELV"]).contains(a.type)}).pluck("id").value();0<a.length&&d.push(f.get_groups(a).then(function(a){g(_.chain(a.data).pluck("structure_id").uniq().value());
l(a.data)}));a=_.chain(c.current_user.profiles).select(function(a){return _("ADM DIR DOC ETA EVS ORI".split(" ")).contains(a.type)}).pluck("structure_id").uniq().value();_(a).isEmpty()||(g(a),d.push(f.get_groups_of_structures(a).then(function(a){l(a.data)})));e.all(d).then(function(){for(b=_(b).uniq();0<b.length;)f.get_users(b.splice(0,200)).then(function(a){c.eleves=c.eleves.concat(_(a.data).map(function(a){a.avatar=k(a.avatar);a.excluded=!1;a.relevant=_(c.relevant_to).contains(a.id);var b=_.chain(a.groups).map(function(a){return _(c.groups).findWhere({id:a.group_id,
...
n=_(g).where({type:"ELV"}),0<n.length?f.get_users(_(n).pluck("user_id")):d.resolve({no_pupils:!0})):d.resolve({no_profile:!0})},function(a){return d.reject(a)}).then(function(b){n=_(n).indexBy("user_id");a.push(_(b.data).map(function(a){var c=1<b.data.length?"s":"";a.type="Autre"+c+" \u00e9l\u00e8ve"+c;return a}));l=_(g).where({type:"ENS"});return 0<l.length?f.get_users(_(l).pluck("user_id")):d.resolve()},function(a){return d.reject(a)}).then(function(b){l=_(l).indexBy("user_id");_(g).where({type:"PRI"});
a.push(_(b.data).map(function(a){a.type=e[l[a.id].type].name+" de l'\u00e9l\u00e8ve";f.get_subjects(_(a.groups).pluck("subject_id")).then(function(b){a.actual_subjects=b.data});return a}));return d.resolve()},function(a){return d.reject(a)}).then(function(){return d.resolve(_.chain(a).flatten().uniq(function(a){return a.id}).value())})});return b}}})}]);
...
resolve:{uids:function(){return b},onglet:function(){return _(g).isNull()?{name:""}:g},all_onglets:function(){return p}},controller:["$scope","$uibModalInstance","$q","Droits","APIs","URL_ENT","DEFAULT_RIGHTS_ONGLET","UID","uids","onglet","all_onglets",function(a,b,d,e,f,g,h,k,m,n,p){a.$ctrl=a;a.uids=m;a.onglet=n;a.all_onglets=p;a.onglet.delete=!1;a.valid_name=!0;_(a.onglet).has("id")||_(a.onglet).has("ids")?e.query({onglet_id:a.onglet.id,"onglets_ids[]":a.onglet.ids}).$promise.then(function(b){a.droits=
_(b).map(function(a){return new e(a)})},function(a){}):(a.droits=[new e({uid:k,read:!0,write:!0,manage:!0})],a.droits.push(new e({uid:m,read:!0,write:!0,manage:!1})),a.droits=a.droits.concat(_(h).map(function(a){a=new e(a);a.dirty={profile_type:!0,read:!0,write:!0,manage:!0};return a})));1==m.length?c.get({id:m[0]}).$promise.then(function(b){a.eleve=b;a.eleve.query_people_concerned_about(m[0]).then(function(b){a.concerned_people=b},function(a){})},function(a){}):c.get({id:k}).$promise.then(function(b){a.concerned_people=
[b];return f.get_users(m)},function(a){}).then(function(b){a.concerned_people=a.concerned_people.concat(b.data)},function(a){});a.name_validation=function(){var b=_.chain(a.all_onglets).reject(function(b){return!_(a.onglet).isNull()&&a.onglet.id==b.id}).pluck("name").value();a.valid_name=!_(b).includes(a.onglet.name);return a.valid_name};a.ok=function(){b.close({onglet:a.onglet,droits:a.droits})};a.delete=function(){swal({title:"\u00cates-vous sur ?",text:"L'onglet ainsi que toutes les saisies et droits associ\u00e9s seront d\u00e9finitivement supprim\u00e9s !",
type:"warning",showCancelButton:!0,confirmButtonColor:"#3085d6",confirmButtonText:"Oui, je confirme !",cancelButtonColor:"#d33",cancelButtonText:"Annuler"}).then(function(b){a.onglet.delete="cancel"!=b.dismiss;a.ok()})};a.cancel=function(){b.dismiss()}}]}).result.then(function(a){var c=null,k="rien";_(g).isNull()?(k="created",c=d.save({uids:b,name:a.onglet.name}).$promise):a.onglet.delete?(k="deleted",a.onglet["ids[]"]=a.onglet.ids,delete a.onglet.ids,c=d.delete(a.onglet).$promise):c=a.onglet.dirty?
d.update(a.onglet).$promise:e.resolve(a.onglet);c.then(function(c){var d=c;void 0==c.ids&&(void 0==c.id?(d=angular.copy(c[0]),delete d.id,delete d.uid_student,d.ids=c.map(function(a){return a.id})):d.ids=[c.id]);d.action=k;"deleted"!=k&&_.chain(a.droits).reject(function(a){return"created"==k&&_(a).has("uid")&&(a.uid==h||a.uid==b)}).each(function(a){a.to_delete&&(_(a).has("id")||_(a).has("ids"))?f.delete(a):!a.to_delete&&a.dirty&&"..."!=a.uid&&"..."!=a.profile_type&&"..."!=a.sharable_id&&_(a.dirty).reduce(function(a,
b){return a||b},!1)&&(a.onglets_ids=d.ids,(_(a).has("id")||_(a).has("ids")?f.update(a):f.save(a)).$promise.then(function(a){return!0},function(a){return!1}))});n(d)},function(){})},function(){})}}]);

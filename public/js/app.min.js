...
"/api/structures/"+b)});this.get_structures=_.memoize(function(b){return a.get(f+"/api/structures/",{params:{"id[]":b}})});this.query_common_onglets_of=function(a){return d.query({"uids[]":a}).$promise.then(function(b){return e.resolve(_.chain(b).reject(function(a){return _(a).isEmpty()}).map(function(a){return _(a).toArray()}).flatten().groupBy("name").toArray().select(function(b){return b.length==a.length}).flatten().groupBy("name").value())},function(a){return e.resolve({})})}}]);

...
_(a.data).pluck("uid_eleve");return c.current_user.get_actual_groups()},function(a){}).then(function(a){var b=[],e=[],l=function(a){c.groups=c.groups.concat(_(a).select(function(a){return"GPL"==a.type||void 0!=_(a.users).findWhere({type:"ELV"})}));c.groups=_(c.groups).uniq(function(c){return a.group_id});f.get_grades(_.chain(c.groups).pluck("grades").flatten().pluck("grade_id").value()).then(function(a){c.grades=a.data},function(a){});b=b.concat(_.chain(a).pluck("users").flatten().compact().select(function(a){return"ELV"==
a.type}).pluck("user_id").value())},g=function(a){1<a.length&&f.get_structures(a).then(function(a){c.structures=c.structures.concat(a.data);c.structures=_(c.structures).uniq(function(a){return a.id})})};_.chain(c.current_user.profiles).pluck("type").contains("ELV").value()&&b.push(c.current_user.id);b=b.concat(_(c.current_user.children).pluck("child_id"));a=_.chain(a).reject(function(a){return _(["ELV"]).contains(a.type)}).pluck("id").value();0<a.length&&e.push(f.get_groups(a).then(function(a){g(_.chain(a.data).pluck("structure_id").uniq().value());
l(a.data)}));a=_.chain(c.current_user.profiles).select(function(a){return _("ADM DIR DOC ETA EVS ORI".split(" ")).contains(a.type)}).pluck("structure_id").uniq().value();_(a).isEmpty()||(g(a),e.push(f.get_groups_of_structures(a).then(function(a){l(a.data)})));d.all(e).then(function(){for(b=_(b).uniq();0<b.length;)f.get_users(b.splice(0,200)).then(function(a){c.eleves=c.eleves.concat(_(a.data).map(function(a){a.avatar=k(a.avatar);a.excluded=!1;a.relevant=_(c.relevant_to).contains(a.id);var b=_.chain(a.groups).map(function(a){return _(c.groups).findWhere({id:a.group_id,

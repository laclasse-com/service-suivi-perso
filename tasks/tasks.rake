...
task :install do 
  ...
  FileUtils.mkdir File.join(APP_ROOT, STORAGE_DIR)    if !File.exist? File.join(APP_ROOT, STORAGE_DIR)
  FileUtils.mkdir File.join(APP_ROOT, TEMP_DIRECTORY) if !File.exist? File.join(APP_ROOT, TEMP_DIRECTORY)
end
